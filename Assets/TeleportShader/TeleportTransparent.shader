...
Shader "HOTATE/TeleportTransparent"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			h2d_const HSConst(InputPatch<v2h,INPUT_PATCH_SIZE> i)
			{
				...
				float tessfactor = lerp(1,_tess,step(_Slider,maxp));
			...
			void geom(triangle d2g input[3],inout TriangleStream<g2f> outStream)
            {
				...
				float4 bp = cp;// + float4(PerlinNoise(mul(UNITY_MATRIX_M,cp)).xxx * saturate(cp[_dir]-_Slider-_sv1) * _noisepower,0);
				///*
